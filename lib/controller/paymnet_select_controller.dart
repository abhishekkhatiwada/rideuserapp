...
import 'package:parkwell/constant/send_notification.dart';
import 'package:parkwell/constant/show_toast_dialog.dart';
import 'package:parkwell/model/order_model.dart';
import 'package:parkwell/model/payment_method_model.dart';
import 'package:parkwell/model/user_model.dart';
import 'package:parkwell/model/wallet_transaction_model.dart';
import 'package:parkwell/payment/MercadoPagoScreen.dart';
import 'package:parkwell/payment/PayFastScreen.dart';
import 'package:parkwell/payment/RazorPayFailedModel.dart';
import 'package:parkwell/payment/getPaytmTxtToken.dart';
import 'package:parkwell/payment/paystack/pay_stack_screen.dart';
import 'package:parkwell/payment/paystack/pay_stack_url_model.dart';
import 'package:parkwell/payment/paystack/paystack_url_genrater.dart';
import 'package:parkwell/payment/stripe_failed_model.dart';
import 'package:parkwell/themes/app_them_data.dart';
import 'package:parkwell/ui/my_booking/parking_ticket_screen.dart';
import 'package:parkwell/utils/fire_store_utils.dart';
import 'package:paytm_allinonesdk/paytm_allinonesdk.dart';
import 'package:razorpay_flutter/razorpay_flutter.dart';

import '../payment/PaywellScreen.dart';
...
class PaymentSelectController extends GetxController {
    ...
    await FireStoreUtils().getPayment().then((value) {
      if (value != null) {
        ...
        if(paymentModel.value.strip!.enable=="true") {
          ...
          Stripe.merchantIdentifier = 'PayWell';
  ...
  double calculateAmount() {
    ...
    double famount=(double.parse(orderModel.value.subTotal.toString()) - double.parse(couponAmount.toString())) + double.parse(taxAmount.value);
    famount+=Constant.serviceFee;
    famount+=Constant.insuranceFee;
    return famount;
  }
  ...
  void initPayPal() async {
    ...
    await _flutterPaypalNativePlugin.init(
      ...
      returnUrl: "com.parkwell.customer://paypalpay",
  ...
  flutterWaveInitiatePayment({required BuildContext context, required String amount}) async {
    final flutterWave = Flutterwave(
      ...
      customization: Customization(title: "PayWell"),
      txRef: _ref!,
  ...
  Future<void> processPaywellPayment(
      {
    ...
    try {
      ...
      final String baseUrl = 'https://wallet.silkinv.com/api/merchant/process-payment';
      var request = http.Request('POST', Uri.parse(baseUrl));
      final String transactionId = '${DateTime.now().millisecondsSinceEpoch}';
      final String? vendorKey = paymentModel.value.paywell!.vendor_key;
      final int amountF=int.parse(amount)*100;
      request.body = json.encode({
        "vendor_key": vendorKey,
        "transaction_id":transactionId,
        "response_url": '${Constant.globalUrl}payment/success',
        "cancellation_url": '${Constant.globalUrl}payment/failure',
        "amount": amountF,
        "title": title,
      ...
      if (response.statusCode == 200) {
          ...
          final String redirectUrl = responseData['redirect_url'];
            final String processId = responseData['process_id'];
            final String status = responseData['status'];
            Get.to(() => PaywellScreen(initialURl: redirectUrl));
          log('Amount : $amountF');
      ...
      log('Exception occurred while processing payment: $e');
    }
  }
